...
palettable.parseSCSS = function(content, fileIdentifier, matchesArray) {
  ...
  while (match = regex.exec(content)) {
    ...
    if (color) {
      matchesArray.push({ fileName: fileIdentifier, variableName: name, tinycolor: color,  clase: '' });
    } else {
      console.log('Invalid color found ' + name + ':' + color);
    }
  }

  matchesArray.push({ fileName: '', variableName: '', tinycolor: color, clase: 'clearfix '});
...
palettable.isGrayish = function(color) {
  return color.toHsv().s < .095;
};

palettable.defaultHoverColorFunction = function(color) {
  return color.clone().brighten(10);
};

palettable.defaultTintColorFunction = function(color) {
  if (palettable.isGrayish(color)) {
    return color;
  } else {
    return tinycolor({
      h: color.toHsv().h,
...
palettable.defaultShadeColorFunction = function(color) {
